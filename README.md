...
行列30 Classic+ 保留 Rime 帶「☯」標記的連打詞句學習能力；這類候選會進入用戶詞典並按使用頻率調整，但不影響原生碼表順位。你可以連續輸入原生單字或原生詞組的字碼，讓 Rime 自動組詞、組句，並記住你的選擇。
...
這類學習結果寫入 Rime 用戶詞典，例如 `array30.userdb`。它會影響連打詞句，但不會反過來改動原生單字碼表和原生詞組碼表的固定順位。
